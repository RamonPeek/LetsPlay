...
The subsequent GitHub Pages preview at `https://ramonpeek.github.io/LetsPlay/` uses its own domain/base path for metadata and assets and defaults to noindex. Pages uses the generated HTML redirect fallback rather than applying `_redirects` as HTTP 301 rules. The production-domain migration checks above still apply when replacing the original business site.
